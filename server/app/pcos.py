...
# PCOS (noise), so cycle length is deliberately excluded — irregularity carries
# the menstrual signal. Dropping it leaves AUC unchanged (0.88 / 0.82).
FULL_FEATURES = [
    ("age", "Age (yrs)"),
    ("bmi", "BMI"),
    ("irregular", "Cycle(R/I)"),          # 0 regular / 1 irregular in our encoding
...
# Human labels for the raised-risk factors, for explanations.
FACTOR_LABELS = {
...
def _dataset_encode(df, feature_map):
    """Map the raw clinical columns onto the app's feature encoding."""
    import pandas as pd
    ...
    for key, col in feature_map:
        ...
        if key == "irregular":
            # dataset: 2 = regular, 4/5 = irregular → 1 when irregular
            s = (s >= 3).astype(float)
        out[key] = s
    X = pd.DataFrame(out)
...
# ---------- rule-based fallback (transparent, no dataset needed) ----------
...
def _top_factors(features_dict, feature_map, k=3):
    """The raised-risk factors this user actually has, most salient first."""
    present = []
