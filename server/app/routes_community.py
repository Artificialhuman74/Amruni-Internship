...
# ---------- moderation ----------
# MVP automated pre-moderation: a blocklist catches clearly disallowed content
# (self-harm encouragement, slurs) and keeps it out of the public feed;
# everything else auto-approves. A real deployment would route to a
# moderation queue or a hosted content-safety API here — this is the
# integration point, mirroring how send_sms() stands in for a real SMS
# gateway in auth.py. Reports that accumulate on a post also pull it down
# automatically (see report_post below).
BLOCKED_TERMS = (
...
def get_or_create_anon_handle(db: sqlite3.Connection, user_id: int) -> str:
    ...
    for _ in range(20):
        ...
        try:
            ...
            continue  # handle collision, try again
...
def _visible_posts_clause(db: sqlite3.Connection, viewer_id: int, life_stage: str) -> tuple[str, list]:
    """WHERE-clause fragment + params excluding blocked authors and, for
    adolescent accounts, restricted-tag posts. Age-gating is enforced here
    (server-side) regardless of what the client already filtered client-side."""
...
def share_journal(entry_id: str, body: ShareJournalBody, user: dict = Depends(current_user)):
    """Publishes a COPY of the journal entry as a community post and links
    back via shared_as_post_id — the private original is never deleted or
    forked, so editing the journal later doesn't rewrite the public post."""
    post_id = new_id("post")
    now = utcnow_iso()
    with get_db() as db:
        entry = db.execute(
...
def block_author(post_id: str, user: dict = Depends(current_user)):
    """Blocks whoever wrote this post. The author's id is resolved server-side
    from the post and never sent to the client, so this works identically
    whether the post was anonymous or not."""
    with get_db() as db:
