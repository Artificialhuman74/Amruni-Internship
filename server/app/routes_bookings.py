...
def confirm_payment(payment_id: str, body: ConfirmBody, user: dict = Depends(current_user)):
    """Settle a payment she made herself.

    The decorator belongs here and not on `settle_payment`. When
    `settle_payment` was extracted so the care link could share it, the route
    decorator was left behind on the helper — which registered a path whose
    parameters are a database handle and two sqlite rows, and left this, the
    real endpoint, unrouted. Confirming a payment 422'd, so no booking could
    ever reach `confirmed`, no meeting link was ever generated, and the whole
    consultation flow dead-ended at the payment sheet.
    """
    with get_db() as db:
        payment = db.execute(
...
def create_room(body: RoomBody, user: dict = Depends(current_user)):
    """The in-app call screen asks for a room; hand it the appointment's real
    Meet link so both paths converge on the same meeting."""
    with get_db() as db:
