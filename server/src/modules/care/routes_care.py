...
from ...database.db import get_db, new_id, to_12h, utcnow_iso
...
def share_slots(token: str, doctorId: int | None = None):
    """Open slots the caretaker can book into."""
    with get_db() as db:
...
def book_via_share(token: str, body: BookBody):
    ...
    is created `pending_payment`, then it waits for her to confirm.
    """
    with get_db() as db:
        share = db.execute("SELECT * FROM care_shares WHERE token = ?", (token,)).fetchone()
