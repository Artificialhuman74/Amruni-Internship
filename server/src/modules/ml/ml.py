...
(each cycle predicted from the woman's preceding cycles).
...
import joblib
import numpy as np
from ...config.settings import DB_PATH, FEDCYCLE_PATH
...
def _find_dataset(override_path: str | None, name: str) -> Path:
    if override_path:
...
# Resolve FEDCYCLE_PATH
resolved_fedcycle_path = _find_dataset(FEDCYCLE_PATH, _FEDCYCLE_NAME)
...
def load_fedcycle():
    ...
    if not resolved_fedcycle_path.exists():
        return None, None
    try:
        import pandas as pd
    ...
    import pandas as pd
    df, _ = __import__("pyreadstat").read_sav(str(resolved_fedcycle_path))
    df = df.copy()
...
def symptom_forecast(day_logs: dict, cycle_starts: list[str], cycle_len: float, period_len: int) -> dict:
    """P(symptom | phase bucket) blended: user's own logged frequencies with population priors."""
